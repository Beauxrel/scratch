...
bool printStats = false;    ///< Flag to enable/disable printing statistics for each node
...
uint8_t nSTAs = 0;      ///< Number of STAs in the network in current iteration
...
CwTrace(std::string context, uint32_t cw, uint8_t /* linkId */)
{
    NS_LOG_INFO("CW time=" << Simulator::Now() << " node=" << ContextToNodeId(context)
                           << " val=" << cw);
    // if (tracing)
    if (tracing && ContextToNodeId(context) == 0 && Simulator::Now().GetSeconds() >= 3.0)
    {
        cwTraceFile <<"nSTAs="<<+nSTAs<< ";time=" << Simulator::Now().GetSeconds() << ";" <<"node="<< ContextToNodeId(context) << ";" <<"newCw="<< cw << ";" <<
                    std::endl;
    }
}
                 ...
                 double distance, double apTxPowerDbm, double staTxPowerDbm, Time pktInterval)
{
  ...
  std::string txop = StaticCast<WifiNetDevice>(wifiNodes.Get(0)->GetDevice(0))->GetMac()->GetQosSupported() ? "BE_Txop" : "Txop";
  // Trace CW evolution
  ...
  Simulator::Schedule (Seconds (3), &RestartCalc);
  Simulator::Stop (Seconds (3) + duration);
...
int main (int argc, char *argv[])
{
  ...
  uint32_t nMaxStas = 40;                 ///< Maximum number of STAs to end with
  ...
  double duration = 20;                  ///< duration (in seconds) of each simulation run (i.e. per trial and per number of stations)
  ...
  if (tracing)
    {
      cwTraceFile.open ("wifi-cw-trace.out");
      if (!cwTraceFile.is_open ())
        {
          NS_FATAL_ERROR ("Failed to open file wifi-cw-trace.out");
        }
      backoffTraceFile.open ("wifi-backoff-trace.out");
      if (!backoffTraceFile.is_open ())
        {
          NS_FATAL_ERROR ("Failed to open file wifi-backoff-trace.out");
        }
      phyTxTraceFile.open ("wifi-phy-tx-trace.out");
      if (!phyTxTraceFile.is_open ())
        {
          NS_FATAL_ERROR ("Failed to open file wifi-phy-tx-trace.out");
        }
      macTxTraceFile.open ("wifi-mac-tx-trace.out");
      if (!macTxTraceFile.is_open ())
        {
          NS_FATAL_ERROR ("Failed to open file wifi-mac-tx-trace.out");
        }
      macRxTraceFile.open ("wifi-mac-rx-trace.out");
      if (!macRxTraceFile.is_open ())
        {
          NS_FATAL_ERROR ("Failed to open file wifi-mac-rx-trace.out");
        }
      socketSendTraceFile.open ("wifi-socket-send-trace.out");
      if (!socketSendTraceFile.is_open ())
        {
          NS_FATAL_ERROR ("Failed to open file wifi-socket-send-trace.out");
        }
    }
  ...
  for (uint32_t n = nMinStas; n <= nMaxStas; n += nStepSize)
    {
      std::cout << "\nNumber of stations in this trial: " << n << "; Max STAs: " << nMaxStas << "; Step size: " << nStepSize << std::endl;
      nSTAs = n;
      averageThroughput = 0;
      double throughput;
      for (uint32_t runIndex = 0; runIndex < trials; runIndex++)
        {
          ...
          associated.clear ();
          throughput = 0;
          std::cout << phyModeStr.str () << " for " << n << " nodes " << std::endl;
